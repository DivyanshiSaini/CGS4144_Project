...
#copy the data
copydataframe_df <- final_mapped_df

# remove the 2 columns
copydataframe_df <- copydataframe_df[, -which(names(copydataframe_df) == "Ensembl")]
copydataframe_df <- copydataframe_df[, -which(names(copydataframe_df) == "first_mapped_symbol_id")]


#load the library
library(dplyr)

# Specify the columns to log scale
columns_to_log_scale <- names(copydataframe_df)[2:ncol(copydataframe_df)]

# Log scale the selected columns
log_scaled <- copydataframe_df %>%
  mutate(across(all_of(columns_to_log_scale), ~log(. + 1), .names = "log_{.col}"))


# Calculate per-gene median expression ranges
gene_median_ranges <- apply(log_scaled[, -1], 2, function(x) diff(range(x)))
...
# Create a density plot of median expression ranges
ggplot(data = data.frame(Median_Range = gene_median_ranges), aes(x = Median_Range)) +
...
matrix_size <- dim(copydataframe_df)
num_genes <- matrix_size[2]  # Number of columns represents the number of genes


# meta file columns to rows 
